...
def display_single_game(game_data: Dict, game_number: int):
    """Fragment for displaying a single game - isolates edit operations."""
    ...
    with st.container(border=True):
        ...
        with col2:
            if st.button("", key=f"edit_game_{game_id}", type="secondary", 
                        icon=":material/edit:", use_container_width=True):
                current_editing = st.session_state.get(f"editing_game_{game_id}", False)
                st.session_state[f"editing_game_{game_id}"] = not current_editing
                st.rerun()
        
        with col3:
            if st.button("", key=f"delete_game_{game_id}", type="secondary", 
                        icon=":material/delete:", use_container_width=True):
                current_state = st.session_state.get(f"confirm_delete_{game_id}", False)
                st.session_state[f"confirm_delete_{game_id}"] = not current_state
                st.rerun()
        ...
        if st.session_state.get(f"confirm_delete_{game_id}", False):
            ...
            with col_confirm:
                if st.button("Yes, Delete Game", key=f"confirm_delete_yes_{game_id}", 
                           type="primary", icon=":material/delete:", use_container_width=True):
            ...
            with col_cancel:
                if st.button("Cancel", key=f"confirm_delete_no_{game_id}", 
                           type="secondary", icon=":material/cancel:", use_container_width=True):
                    if f"confirm_delete_{game_id}" in st.session_state:
                        del st.session_state[f"confirm_delete_{game_id}"]
                    st.rerun()
        ...
        with col1:
            if game_data.get('scores'):
                ...
                for i, score_info in enumerate(scores_data):
                    ...
                    score_text.append(f"{medal} {score_info['player_name']} : {score_info['score']} pts")
        ...
        # Edit form (only render if in edit mode)
        if st.session_state.get(f"editing_game_{game_id}", False):
            display_edit_form(game_data)

@st.fragment  
def display_edit_form(game_data: Dict):
    """Fragment for edit form to isolate edit operations."""
    game_id = game_data['id']
    ...
    with st.form(f"edit_game_form_{game_id}", border=False):
        ...
        with col1:
            ...
            edit_game_date = st.date_input("Game Date", value=current_date, 
                                         format="DD.MM.YYYY", key=f"edit_date_{game_id}")
        
        with col2:
            notes = str(game_data.get('notes', '')) if game_data.get('notes') else ""
            edit_notes = st.text_area("Notes", value=notes, key=f"edit_notes_{game_id}", height=80)
                ...
                for col_idx in range(2):
                    ...
                    if score_idx < len(scores_list):
                        with score_cols[col_idx]:
                            ...
                            new_score = st.number_input(f"🎮 {player_name}", 
                                                       min_value=-1000, max_value=10000,
                                                       value=current_score, step=1,
                        ...
                        with settings_cols[col_idx]:
                            ...
                            if setting_type == 'number':
                                try:
                                    ...
                                    current_num = 0
                                new_value = st.number_input(setting_name, min_value=0, 
                                                           value=current_num, step=1,
                                ...
                                new_value = st.toggle(setting_name, value=current_bool,
                                                    key=f"edit_setting_{game_id}_{setting_id}")
                                edit_setting_values[setting_id] = str(new_value)
                                ...
                                try:
                                    ...
                                    current_minutes = 60
                                new_value = st.number_input(f"{setting_name} (minutes)", 
                                                           min_value=1, max_value=1440,
                                                           value=current_minutes, step=1,
                                                           key=f"edit_setting_{game_id}_{setting_id}")
                                if new_value >= 1:
                                    edit_setting_values[setting_id] = str(new_value)
                                ...
                                if len(list_items_df) > 0:
                                    ...
                                    try:
                                        ...
                                        current_index = 0
                                    new_value = st.selectbox(setting_name, options=options, 
                                                           index=current_index,
                                                           key=f"edit_setting_{game_id}_{setting_id}")
        ...
        with col_save:
            save_game = st.form_submit_button("Save Changes", type="primary", 
        ...
        if save_game:
            if db.update_game_in_database(game_id, edit_game_date, edit_player_scores, 
                                        edit_setting_values, edit_notes or ""):
...
def display_new_game_form():
    ...
    if len(active_players) == 0:
        st.warning("No active players found. Please add and activate players in the Settings page.")
        return
    ...
    with st.form(f"new_game_form_{st.session_state.game_form_counter}", border=True):
        ...
        with col1:
            game_date = st.date_input("Game Date", value=date.today(), format="DD.MM.YYYY")
        
        with col2:
            notes = st.text_area("Notes (optional)", 
                               placeholder="Optional notes about the game...", height=80)
            ...
            for col_idx in range(2):
                ...
                if player_idx < len(active_players):
                    with score_cols[col_idx]:
                        ...
                        score = st.number_input(f"🎮 {player_name}", min_value=-20, max_value=200,
                                              value=0, step=1,
                    ...
                    if setting_idx < len(active_settings):
                        with settings_cols[col_idx]:
                            ...
                            if setting_type == "number":
                                ...
                                if len(list_items_df) > 0:
                                    options = list_items_df["value"].tolist()
                                    value = st.selectbox(setting_name, options=options,
                                                       key=f"setting_{setting_id}_{st.session_state.game_form_counter}")
        ...
        # Submit button
        submit_game = st.form_submit_button("Save Game", use_container_width=True, 
        ...
        if submit_game:
            ...
            if len(non_zero_scores) == 0:
                ...
                if db.add_game_to_database(game_date, player_scores, setting_values, notes):
                    del st.session_state[submission_key]
...
tab1, tab2 = st.tabs([s.center(16, "\u2001") for s in ["Game History", "Record New Game"]])
...
with tab1:
    ...
    if games_data:
        ...
        st.info("No games recorded yet. Use the 'Record New Game' tab to add your first game!")
